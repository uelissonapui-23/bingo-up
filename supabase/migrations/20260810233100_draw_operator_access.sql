-- Etapa 10.1: operador de sorteio por convite e evento, com menor privilégio.

create table if not exists public.draw_operator_invitations (
...
create index if not exists draw_operator_invitations_workspace_idx on public.draw_operator_invitations(workspace_id,created_at desc);
create index if not exists draw_operator_invitations_email_idx on public.draw_operator_invitations(lower(email),status);

create table if not exists public.event_draw_operator_assignments (
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  ...
  primary key(event_id,operator_user_id)
);
create index if not exists event_draw_operator_user_idx on public.event_draw_operator_assignments(operator_user_id,is_active,event_id);
create index if not exists event_draw_operator_workspace_idx on public.event_draw_operator_assignments(workspace_id,event_id,is_active);

alter table public.draw_operator_invitations enable row level security;
alter table public.event_draw_operator_assignments enable row level security;
...
returns boolean language sql stable security definer set search_path=public as $$
  select public.is_platform_admin() or exists(
    ...
    where a.event_id=target_event_id and a.operator_user_id=target_user_id and a.is_active
      and wm.status='active' and wm.role::text='draw_operator'
  );
$$;
...
create policy draw_operator_invitations_admin_read on public.draw_operator_invitations for select to authenticated
using(public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin']::public.workspace_role[]));
create policy event_draw_operator_assignments_read on public.event_draw_operator_assignments for select to authenticated
using(public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin']::public.workspace_role[]) or operator_user_id=auth.uid());

create or replace function public.create_draw_operator_invitation(target_workspace_id uuid,target_email text,target_event_ids uuid[] default '{}'::uuid[])
...
begin
  ...
  if coalesce(cardinality(target_event_ids),0)=0 then raise exception 'at least one event is required'; end if;
  ...
  update public.draw_operator_invitations set status='revoked' where workspace_id=target_workspace_id and lower(email)=clean_email and status='pending';
  insert into public.draw_operator_invitations(workspace_id,email,event_ids) values(target_workspace_id,clean_email,target_event_ids) returning token into invite_token;
  perform public.log_audit(target_workspace_id,'draw_operator.invited','draw_operator_invitation',invite_token::text,jsonb_build_object('email',clean_email,'event_ids',target_event_ids));
  return invite_token;
end; $$;
revoke all on function public.create_draw_operator_invitation(uuid,text,uuid[]) from public;
grant execute on function public.create_draw_operator_invitation(uuid,text,uuid[]) to authenticated;

create or replace function public.get_draw_operator_invitation(invite_token uuid)
...
language sql stable security definer set search_path=public as $$
  ...
  from public.draw_operator_invitations i join public.workspaces w on w.id=i.workspace_id where i.token=invite_token;
$$;
revoke all on function public.get_draw_operator_invitation(uuid) from public;
grant execute on function public.get_draw_operator_invitation(uuid) to authenticated;
...
begin
  ...
  insert into public.workspace_members(workspace_id,user_id,role,status) values(i.workspace_id,auth.uid(),'draw_operator','active')
  on conflict(workspace_id,user_id) do update set role='draw_operator',status='active',updated_at=now();
...
begin
  if not public.has_workspace_role(target_workspace_id,array['organizer_owner','organizer_admin']::public.workspace_role[]) then raise exception 'access denied'; end if;
  if coalesce(cardinality(target_event_ids),0)=0 then raise exception 'at least one event is required'; end if;
  if not exists(select 1 from public.workspace_members where workspace_id=target_workspace_id and user_id=target_operator_user_id and role::text='draw_operator' and status='active') then raise exception 'operator not active'; end if;
  if exists(select 1 from unnest(target_event_ids) e_id where not exists(select 1 from public.events e where e.id=e_id and e.workspace_id=target_workspace_id)) then raise exception 'invalid event assignment'; end if;
  ...
  foreach ev in array target_event_ids loop
    insert into public.event_draw_operator_assignments(workspace_id,event_id,operator_user_id,is_active) values(target_workspace_id,ev,target_operator_user_id,true)
    on conflict(event_id,operator_user_id) do update set is_active=true,updated_at=now();
  end loop;
  perform public.log_audit(target_workspace_id,'draw_operator.assignments_updated','workspace_member',target_operator_user_id::text,jsonb_build_object('event_ids',target_event_ids));
end; $$;
revoke all on function public.set_draw_operator_event_assignments(uuid,uuid,uuid[]) from public;
grant execute on function public.set_draw_operator_event_assignments(uuid,uuid,uuid[]) to authenticated;
...
begin
  if not public.has_workspace_role(target_workspace_id,array['organizer_owner','organizer_admin']::public.workspace_role[]) then raise exception 'access denied'; end if;
  if target_status not in ('active','suspended','revoked') then raise exception 'invalid operator status'; end if;
  update public.workspace_members set status=target_status,updated_at=now() where workspace_id=target_workspace_id and user_id=target_operator_user_id and role::text='draw_operator';
  ...
  perform public.log_audit(target_workspace_id,'draw_operator.status_changed','workspace_member',target_operator_user_id::text,jsonb_build_object('status',target_status));
end; $$;
revoke all on function public.set_draw_operator_membership_status(uuid,uuid,public.membership_status) from public;
grant execute on function public.set_draw_operator_membership_status(uuid,uuid,public.membership_status) to authenticated;
...
language sql stable security definer set search_path=public as $$
  select wm.user_id,p.display_name,u.email,wm.status,
    coalesce((select array_agg(a.event_id order by a.event_id) from public.event_draw_operator_assignments a where a.workspace_id=wm.workspace_id and a.operator_user_id=wm.user_id and a.is_active),'{}'::uuid[])
  from public.workspace_members wm left join public.profiles p on p.id=wm.user_id left join auth.users u on u.id=wm.user_id
  where wm.workspace_id=target_workspace_id and wm.role::text='draw_operator'
...
$$;
revoke all on function public.list_draw_operator_team(uuid) from public;
grant execute on function public.list_draw_operator_team(uuid) to authenticated;

-- Atualização estritamente limitada à aparência/visibilidade do painel público.
...
begin
  ...
  update public.event_settings set
    ...
    updated_at=now()
  where event_id=e.id;
  perform public.log_audit(e.workspace_id,'public_panel.operator_updated','event',e.id::text,jsonb_build_object('patch',target_patch));
end; $$;
...
-- SELECTs mínimos: somente o evento atribuído e dados necessários para sorteio/conferência.
...
create policy events_member_select on public.events for select to authenticated using(
  ...
  or public.seller_has_event_access(id) or public.draw_operator_has_event_access(id)
);
drop policy if exists event_settings_member_select on public.event_settings;
create policy event_settings_member_select on public.event_settings for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[])
  or public.seller_has_event_access(event_id) or public.draw_operator_has_event_access(event_id)
);
drop policy if exists bingo_rule_sets_member_select on public.bingo_rule_sets;
create policy bingo_rule_sets_member_select on public.bingo_rule_sets for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[])
...
create policy physical_cards_member_select on public.physical_cards for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[])
  or public.seller_has_event_access(event_id) or public.draw_operator_has_event_access(event_id)
);
drop policy if exists game_definitions_member_select on public.game_definitions;
create policy game_definitions_member_select on public.game_definitions for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[]) or public.draw_operator_has_event_access(event_id)
);
drop policy if exists card_games_member_select on public.card_games;
create policy card_games_member_select on public.card_games for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[]) or public.draw_operator_has_event_access(event_id)
);
drop policy if exists draw_sessions_member_select on public.draw_sessions;
create policy draw_sessions_member_select on public.draw_sessions for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[]) or public.draw_operator_has_event_access(event_id)
);
drop policy if exists draw_session_games_member_select on public.draw_session_games;
create policy draw_session_games_member_select on public.draw_session_games for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[]) or public.draw_operator_has_event_access(event_id)
);
drop policy if exists draw_numbers_member_select on public.draw_numbers;
create policy draw_numbers_member_select on public.draw_numbers for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[]) or public.draw_operator_has_event_access(event_id)
);
drop policy if exists game_progress_member_select on public.game_progress;
create policy game_progress_member_select on public.game_progress for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[]) or public.draw_operator_has_event_access(event_id)
);
drop policy if exists winner_candidates_member_select on public.winner_candidates;
create policy winner_candidates_member_select on public.winner_candidates for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[]) or public.draw_operator_has_event_access(event_id)
);
drop policy if exists winners_member_select on public.winners;
create policy winners_member_select on public.winners for select to authenticated using(
  public.has_workspace_role(workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[]) or public.draw_operator_has_event_access(event_id)
);

create trigger event_draw_operator_assignments_set_updated_at before update on public.event_draw_operator_assignments for each row execute function public.set_updated_at();


-- As funções abaixo preservam a lógica já estabilizada e apenas ampliam a autorização para operador atribuído ao evento.
...
begin
  ...
  if s.id is null then raise exception 'draw session not found'; end if;
  if not public.has_workspace_role(s.workspace_id,array['organizer_owner','organizer_admin','event_manager']::public.workspace_role[]) and not public.draw_operator_has_event_access(s.event_id) then raise exception 'access denied'; end if;
  return public.evaluate_draw_session_progress(target_session_id);
end; $$;
revoke all on function public.refresh_draw_progress(uuid) from public;
grant execute on function public.refresh_draw_progress(uuid) to authenticated;
