-- Permite que o próprio destinatário veja seus convites pendentes na Central do vendedor.
...
begin
  ...
  select lower(email),email_confirmed_at into current_email,confirmed_at from auth.users where id=auth.uid();
  if current_email is null or confirmed_at is null then raise exception 'confirmed email required'; end if;
  return query
    select i.token,w.name,i.email,
    ...
    where lower(i.email)=current_email and i.status='pending' and i.expires_at>now() and w.is_active
