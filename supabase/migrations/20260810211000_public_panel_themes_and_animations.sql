-- Visual do painel público fica armazenado em event_settings.settings para não acoplar aparência à lógica do sorteio.
...
begin
  ...
  from public.winners w join public.physical_cards pc on pc.id=w.physical_card_id join public.card_games cg on cg.id=w.card_game_id where w.session_id=s.id;
  ...
  return jsonb_build_object(
    ...
    'possible_bingo',detected>0 and confirmed=0,'confirmed_bingo',confirmed>0,'confirmed_winners',confirmed,'winner_labels',winner_labels,
    'win_pattern_name',pattern_name,'appearance',appearance,'updated_at',now()
  );
end; $$;
