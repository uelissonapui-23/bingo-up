...
export type BuyerEventState={event:{id:string;name:string;status:string};public_session_token:string|null;winner_count:number;winner_prizes:string[];latest_win:null|{prize:string;my_winners:number;total_winners:number;buyer_name:string|null};cards:Array<{id:string;code:string;public_token:string;physical_format:number;buyer_name:string|null}>}
