...
export async function findCandidateForCard(sessionId:string,code:string):Promise<WinnerCheck[]>{
 ...
 const [{data:games,error:ge},{data:progress,error:pe},{data:triggers,error:te},{data:sale,error:se}]=await Promise.all([
