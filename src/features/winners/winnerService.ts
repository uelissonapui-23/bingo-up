...
export async function findCandidateForCard(sessionId:string,code:string):Promise<WinnerCheck[]>{
 ...
 const [{data:games,error:ge},{data:progress,error:pe},{data:triggers,error:te},{data:sale},{data:drawn,error:de}]=await Promise.all([
 ...
 ]);if(ge)throw ge;if(pe)throw pe;if(te)throw te;if(de)throw de
