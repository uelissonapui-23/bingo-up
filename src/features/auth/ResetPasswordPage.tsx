...
export function ResetPasswordPage() {
  ...
  async function submit(e: FormEvent) { e.preventDefault(); setError(''); if (password.length < 8) return setError('A senha precisa ter pelo menos 8 caracteres.'); if (password !== confirm) return setError('As senhas não conferem.'); setBusy(true); try { const { error } = await supabase.auth.updateUser({ password }); if (error) setError('Não foi possível alterar a senha. Abra novamente o link de recuperação.'); else setDone(true) } catch { setError('Não foi possível conectar ao serviço de acesso. Tente abrir novamente o link.') } finally { setBusy(false) } }
