...
export function ConfirmEmailPage() {
  ...
  useEffect(() => {
    ...
    async function finishConfirmation() {
      ...
      if (urlError) {
        if (active) setError('O link de confirmação é inválido ou expirou. Solicite um novo cadastro ou tente novamente.')
        return
      }
      ...
      try {
        ...
        if (code) {
          ...
          // No fluxo implícito, detectSessionInUrl processa os tokens do hash.
          // Aguarde a sessão aparecer antes de limpá-la e devolver o usuário ao login.
        ...
        if (active) navigate('/entrar?emailConfirmado=1', { replace: true })
      } catch {
        if (active) setError('Não foi possível concluir a confirmação. O link pode ter expirado. Tente abrir novamente o e-mail de confirmação.')
      }
    }
