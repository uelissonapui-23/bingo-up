...
export function PublicPanelPage(){
  ...
  return <main className="min-h-dvh overflow-hidden bg-slate-950 p-4 text-white sm:p-6 lg:p-8">
    <div className="mx-auto flex min-h-[calc(100dvh-2rem)] max-w-[1800px] flex-col sm:min-h-[calc(100dvh-3rem)] lg:min-h-[calc(100dvh-4rem)]">
      <header className="flex flex-wrap items-end justify-between gap-3 border-b border-white/10 pb-4"><div className="min-w-0"><p className="break-words text-sm font-bold uppercase tracking-[.18em] text-emerald-400 sm:tracking-[.25em]">{state.event_name}</p><h1 className="mt-1 break-words text-2xl font-black sm:text-3xl">{state.round_name}</h1></div><div className="text-right"><p className="text-sm font-bold text-slate-300">{statusLabel}</p><p className="text-xs text-slate-500">Prêmio: {state.win_pattern_name}</p></div></header>
      {state.confirmed_bingo?<PublicWinner count={state.confirmed_winners}/>:state.possible_bingo?<PossibleWinner/>:<div className="grid flex-1 gap-5 py-5 lg:grid-cols-[.72fr_1.28fr]">
        ...
        <section className="rounded-[2rem] border border-white/10 bg-white/[.04] p-4 sm:p-6"><div className="flex flex-wrap items-center justify-between gap-2"><h2 className="text-lg font-black sm:text-2xl">Números que já saíram</h2><span className="text-sm font-bold text-slate-500">{state.total_balls-state.called_count} restantes</span></div><div className="mt-5 grid grid-cols-5 gap-1.5 min-[520px]:grid-cols-10 sm:gap-2 lg:gap-2.5">{Array.from({length:state.total_balls},(_,i)=>i+1).map(n=><div key={n} className={`grid aspect-square place-items-center rounded-lg border text-[clamp(.7rem,1.6vw,1.5rem)] font-black ${called.has(n)?'border-emerald-400 bg-emerald-500 text-slate-950 shadow-[0_0_24px_rgba(52,211,153,.18)]':'border-white/10 bg-black/20 text-slate-600'}`}>{n}</div>)}</div></section>
      </div>}
    </div>
  </main>
}
