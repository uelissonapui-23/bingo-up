...
import { ensureCardConfigDefaults, getCardAssetUrl, getEventCardArtwork, getEventCardGameStyle, listCardTemplates, listRuleSets, saveEventCardArtwork, saveEventCardGameStyle } from '@/features/card-config/cardConfigService'
...
import { DEFAULT_GAME_STYLE, parseCardTemplateOptions, type ArtworkFit, type ArtworkQuality, type CardArtworkOptions, type CardGameFont, type CardGameStyleOptions, type CardTemplateOptions } from '@/domain/cards/templateOptions'
...
export function CardGeneratorPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [ev, rs, ts, bs, artwork, gameStyle] = await Promise.all([
        getEvent(workspaceId, eventId), listRuleSets(workspaceId, eventId), listCardTemplates(workspaceId, eventId), listCardBatches(workspaceId, eventId), getEventCardArtwork(workspaceId,eventId), getEventCardGameStyle(workspaceId,eventId),
      ])
      setEvent(ev); setRules(rs.filter(r => r.is_active)); setTemplates(ts.filter(t => t.is_active)); setBatches(bs); setEventArtwork(artwork); setEventGameStyle(gameStyle)
  ...
  return <div className="space-y-6">
    ...
    <GenerationForm rules={rules} templates={templates} eventArtwork={eventArtwork} eventGameStyle={eventGameStyle} batches={batches} usedByRule={usedByRule} workspaceId={currentWorkspace.id} eventId={eventId} eventName={event.name} initialFormat={searchParams.get('formato')} initialTemplateId={searchParams.get('modelo')} disabled={busy} onBusy={setBusy} onProgress={setProgress} onError={setError} onDone={async message => { setNotice(message); setProgress(null); const [bs,counts]=await Promise.all([listCardBatches(currentWorkspace.id,eventId),Promise.all(rules.map(async r=>[r.id,await countGameDefinitions(r.id)] as const))]);setBatches(bs);setUsedByRule(Object.fromEntries(counts)) }} onArtworkSaved={setEventArtwork} onGameStyleSaved={setEventGameStyle} />
...
function GenerationForm({ rules, templates, eventArtwork, eventGameStyle, batches, usedByRule, workspaceId, eventId, eventName, initialFormat, initialTemplateId, disabled, onBusy, onProgress, onError, onDone, onArtworkSaved, onGameStyleSaved }:{ rules:BingoRuleSet[];templates:CardTemplate[];eventArtwork:CardArtworkOptions|undefined;eventGameStyle:CardGameStyleOptions;batches:CardBatch[];usedByRule:Record<string,number>;workspaceId:string;eventId:string;eventName:string;initialFormat:string|null;initialTemplateId:string|null;disabled:boolean;onBusy:(v:boolean)=>void;onProgress:(v:Progress)=>void;onError:(v:string|null)=>void;onDone:(message:string)=>Promise<void>;onArtworkSaved:(artwork:CardArtworkOptions)=>void;onGameStyleSaved:(style:CardGameStyleOptions)=>void }) {
  ...
  return <Card className="border-red-900/30"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-xl font-black">Criar cartelas</h2><p className="mt-1 text-sm text-slate-600">Defina o formato, deixe a arte do jeito que quer, confira a cartela e escolha a quantidade. Tudo acontece nesta tela.</p></div><StatusBadge tone={needsControlled?'warning':'success'}>{needsControlled?'Verifique a quantidade':'Pronto para gerar'}</StatusBadge></div>
    <div className="mt-5 space-y-6">
      ...
      {template&&<ArtworkQuickEditor workspaceId={workspaceId} eventId={eventId} eventName={eventName} rule={rule} template={template} eventArtwork={eventArtwork} eventGameStyle={eventGameStyle} format={format} series={series} startNumber={startNumber} padding={padding} onSaved={onArtworkSaved}/>}
      {template&&<GameStyleQuickEditor workspaceId={workspaceId} eventId={eventId} eventName={eventName} rule={rule} template={template} eventArtwork={eventArtwork} eventGameStyle={eventGameStyle} format={format} series={series} startNumber={startNumber} padding={padding} onSaved={onGameStyleSaved}/>}
...
function ArtworkQuickEditor({workspaceId,eventId,eventName,rule,template,eventArtwork,eventGameStyle,format,series,startNumber,padding,onSaved}:{workspaceId:string;eventId:string;eventName:string;rule:BingoRuleSet|null|undefined;template:CardTemplate;eventArtwork:CardArtworkOptions|undefined;eventGameStyle:CardGameStyleOptions;format:1|2|3;series:string;startNumber:number;padding:number;onSaved:(artwork:CardArtworkOptions)=>void}){
  ...
  const options:CardTemplateOptions={...templateOptions,artwork:previewArtwork,gameStyle:eventGameStyle}
  // A imagem escolhida é um rascunho do EVENTO, não do modelo. Ao trocar entre
  // 1/2/3 em 1 ou navegar pelos layouts, preserve o arquivo local para que o
  // organizador compare os modelos antes de decidir qual usar. Só sincronize e
  // limpe o rascunho quando a arte persistida do evento realmente mudar.
  useEffect(()=>{const next=eventArtwork;setFit(next?.fit??'cover');setZoom(next?.zoom??1);setOffsetX(next?.offsetX??0);setOffsetY(next?.offsetY??0);setQuality(next?.quality??'standard');setArtFile(null);setArtUrl(current=>{if(current)URL.revokeObjectURL(current);return null})},[eventArtwork])
...
function GameStyleQuickEditor({workspaceId,eventId,eventName,rule,template,eventArtwork,eventGameStyle,format,series,startNumber,padding,onSaved}:{workspaceId:string;eventId:string;eventName:string;rule:BingoRuleSet|null|undefined;template:CardTemplate;eventArtwork:CardArtworkOptions|undefined;eventGameStyle:CardGameStyleOptions;format:1|2|3;series:string;startNumber:number;padding:number;onSaved:(style:CardGameStyleOptions)=>void}){
  ...
  const options:CardTemplateOptions={...parseCardTemplateOptions(template.options),artwork:eventArtwork??parseCardTemplateOptions(template.options).artwork,gameStyle:style}
  ...
  {open&&<div className="mt-4 grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,.9fr)]"><div className="space-y-5">
    <div><p className="text-sm font-black text-slate-200">Cores</p><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><ColorField label="Números" value={style.numberColor} set={v=>update('numberColor',v)}/><ColorField label="Linhas da grade" value={style.gridColor} set={v=>update('gridColor',v)}/><ColorField label="Fundo dos números" value={style.cellBackground} set={v=>update('cellBackground',v)}/><ColorField label="Barra BINGO" value={style.headerBackground} set={v=>update('headerBackground',v)}/><ColorField label="Letras BINGO" value={style.headerTextColor} set={v=>update('headerTextColor',v)}/></div></div>
