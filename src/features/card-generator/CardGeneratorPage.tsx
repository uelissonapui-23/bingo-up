...
function GenerationForm({ rules, templates, usedByRule, workspaceId, eventId, eventName, initialFormat, initialTemplateId, disabled, onBusy, onProgress, onError, onDone, onRefresh }:{ rules:BingoRuleSet[];templates:CardTemplate[];usedByRule:Record<string,number>;workspaceId:string;eventId:string;eventName:string;initialFormat:string|null;initialTemplateId:string|null;disabled:boolean;onBusy:(v:boolean)=>void;onProgress:(v:Progress)=>void;onError:(v:string|null)=>void;onDone:(message:string)=>Promise<void>;onRefresh:()=>Promise<void> }) {
  ...
  return <Card className="border-red-900/30"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-xl font-black">Criar cartelas</h2><p className="mt-1 text-sm text-slate-600">Defina o formato, deixe a arte do jeito que quer, confira a cartela e escolha a quantidade. Tudo acontece nesta tela.</p></div><StatusBadge tone={needsControlled?'warning':'success'}>{needsControlled?'Verifique a quantidade':'Pronto para gerar'}</StatusBadge></div>
    <div className="mt-5 space-y-6">
      ...
      {template&&<ArtworkQuickEditor workspaceId={workspaceId} eventId={eventId} eventName={eventName} rule={rule} template={template} format={format} series={series} startNumber={startNumber} padding={padding} onSaved={onRefresh}/>}
...
function ArtworkQuickEditor({workspaceId,eventId,eventName,rule,template,format,series,startNumber,padding,onSaved}:{workspaceId:string;eventId:string;eventName:string;rule:BingoRuleSet|null|undefined;template:CardTemplate;format:1|2|3;series:string;startNumber:number;padding:number;onSaved:()=>Promise<void>}){
  ...
  async function save(){setSaving(true);setMessage(null);try{const artworkFile=artFile?await optimizeArtwork(artFile,quality):undefined;await updateCardTemplate(workspaceId,eventId,template.id,{name:template.name,format,layoutKey:template.layout_key,orientation:template.orientation,pageSize:template.page_size,bannerPosition:'none',bannerHeightMm:0,isDefault:template.is_default,options,artworkFile});setMessage('Imagem de fundo salva. A visualização já representa o que será usado na cartela.');setArtFile(null);if(artUrl){URL.revokeObjectURL(artUrl);setArtUrl(null)};await onSaved();setOpen(false)}catch(e){setMessage(e instanceof Error?e.message:'Não foi possível salvar a imagem de fundo.')}finally{setSaving(false)}}
