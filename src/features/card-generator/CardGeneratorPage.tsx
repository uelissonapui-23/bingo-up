...
export function CardGeneratorPage() {
  ...
  return <div className="space-y-6">
    ...
    <GenerationForm rules={rules} templates={templates} usedByRule={usedByRule} workspaceId={currentWorkspace.id} eventId={eventId} disabled={busy} onBusy={setBusy} onProgress={setProgress} onError={setError} onDone={async message => { setNotice(message); setProgress(null); await load() }} />
    <BatchList batches={batches} busy={busy} onCancel={async batch => { setBusy(true); setError(null); try { await cancelCardBatch(batch.id, 'Cancelado pelo organizador.'); setNotice('Lote cancelado e suas cartelas parciais removidas.'); await load() } catch (e) { setError(e instanceof Error ? e.message : 'Não foi possível cancelar o lote.') } finally { setBusy(false) } }} />
  </div>
}

function GenerationForm({ rules, templates, usedByRule, workspaceId, eventId, disabled, onBusy, onProgress, onError, onDone }:{ rules:BingoRuleSet[];templates:CardTemplate[];usedByRule:Record<string,number>;workspaceId:string;eventId:string;disabled:boolean;onBusy:(v:boolean)=>void;onProgress:(v:Progress)=>void;onError:(v:string|null)=>void;onDone:(message:string)=>Promise<void> }) {
  ...
  return <Card className="border-emerald-200"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-xl font-black">Novo lote</h2><p className="mt-1 text-sm text-slate-600">O banco revalida cada jogo e impede que a geração viole a política escolhida.</p></div><StatusBadge tone={needsControlled?'warning':'success'}>{needsControlled?'Repetição necessária':'100% único possível'}</StatusBadge></div>
