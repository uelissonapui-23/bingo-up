...
import { ensureCardConfigDefaults, getCardAssetUrl, listCardTemplates, listRuleSets, updateCardTemplate } from '@/features/card-config/cardConfigService'
...
import { parseCardTemplateOptions, type ArtworkFit, type ArtworkQuality, type CardTemplateOptions } from '@/domain/cards/templateOptions'
...
export function CardGeneratorPage() {
  ...
  return <div className="space-y-6">
    ...
    <GenerationForm rules={rules} templates={templates} usedByRule={usedByRule} workspaceId={currentWorkspace.id} eventId={eventId} eventName={event.name} initialFormat={searchParams.get('formato')} initialTemplateId={searchParams.get('modelo')} disabled={busy} onBusy={setBusy} onProgress={setProgress} onError={setError} onDone={async message => { setNotice(message); setProgress(null); const [bs,counts]=await Promise.all([listCardBatches(currentWorkspace.id,eventId),Promise.all(rules.map(async r=>[r.id,await countGameDefinitions(r.id)] as const))]);setBatches(bs);setUsedByRule(Object.fromEntries(counts)) }} onTemplateSaved={updated=>setTemplates(current=>current.map(t=>t.id===updated.id?updated:t))} />
...
function GenerationForm({ rules, templates, usedByRule, workspaceId, eventId, eventName, initialFormat, initialTemplateId, disabled, onBusy, onProgress, onError, onDone, onTemplateSaved }:{ rules:BingoRuleSet[];templates:CardTemplate[];usedByRule:Record<string,number>;workspaceId:string;eventId:string;eventName:string;initialFormat:string|null;initialTemplateId:string|null;disabled:boolean;onBusy:(v:boolean)=>void;onProgress:(v:Progress)=>void;onError:(v:string|null)=>void;onDone:(message:string)=>Promise<void>;onTemplateSaved:(updated:CardTemplate)=>void }) {
  ...
  return <Card className="border-red-900/30"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-xl font-black">Criar cartelas</h2><p className="mt-1 text-sm text-slate-600">Defina o formato, deixe a arte do jeito que quer, confira a cartela e escolha a quantidade. Tudo acontece nesta tela.</p></div><StatusBadge tone={needsControlled?'warning':'success'}>{needsControlled?'Verifique a quantidade':'Pronto para gerar'}</StatusBadge></div>
    <div className="mt-5 space-y-6">
      ...
      {template&&<ArtworkQuickEditor workspaceId={workspaceId} eventId={eventId} eventName={eventName} rule={rule} template={template} format={format} series={series} startNumber={startNumber} padding={padding} onSaved={onTemplateSaved}/>}
...
function ArtworkQuickEditor({workspaceId,eventId,eventName,rule,template,format,series,startNumber,padding,onSaved}:{workspaceId:string;eventId:string;eventName:string;rule:BingoRuleSet|null|undefined;template:CardTemplate;format:1|2|3;series:string;startNumber:number;padding:number;onSaved:(updated:CardTemplate)=>void}){
  const initial=parseCardTemplateOptions(template.options)
  const [open,setOpen]=useState(false)
  const [artFile,setArtFile]=useState<File|null>(null)
  const [artUrl,setArtUrl]=useState<string|null>(null)
  const [fit,setFit]=useState<ArtworkFit>(initial.artwork?.fit??'cover')
  const [zoom,setZoom]=useState(initial.artwork?.zoom??1)
  const [offsetX,setOffsetX]=useState(initial.artwork?.offsetX??0)
  const [offsetY,setOffsetY]=useState(initial.artwork?.offsetY??0)
  const [quality,setQuality]=useState<ArtworkQuality>(initial.artwork?.quality??'standard')
  const [saving,setSaving]=useState(false)
  const [message,setMessage]=useState<string|null>(null)
  const existingUrl=getCardAssetUrl(initial.artwork?.path)
  useEffect(()=>()=>{if(artUrl)URL.revokeObjectURL(artUrl)},[artUrl])
  useEffect(()=>{const next=parseCardTemplateOptions(template.options);setFit(next.artwork?.fit??'cover');setZoom(next.artwork?.zoom??1);setOffsetX(next.artwork?.offsetX??0);setOffsetY(next.artwork?.offsetY??0);setQuality(next.artwork?.quality??'standard')},[template])
  function choose(file:File|null){if(artUrl)URL.revokeObjectURL(artUrl);setArtFile(file);setArtUrl(file?URL.createObjectURL(file):null);setMessage(null);if(file)setOpen(true)}
  const options:CardTemplateOptions={version:1,artwork:(artFile||initial.artwork?.path)?{path:initial.artwork?.path??'',fit,zoom,offsetX,offsetY,quality}:undefined,wildcard:initial.wildcard}
  async function save(){setSaving(true);setMessage(null);try{const artworkFile=artFile?await optimizeArtwork(artFile,quality):undefined;const updated=await updateCardTemplate(workspaceId,eventId,template.id,{name:template.name,format,layoutKey:template.layout_key,orientation:template.orientation,pageSize:template.page_size,bannerPosition:'none',bannerHeightMm:0,isDefault:template.is_default,options,artworkFile});onSaved(updated);setMessage('Imagem de fundo salva e confirmada. Ela será usada nas cartelas geradas e no PDF.');setArtFile(null);if(artUrl){URL.revokeObjectURL(artUrl);setArtUrl(null)};setOpen(false)}catch(e){setMessage(e instanceof Error?e.message:'Não foi possível salvar a imagem de fundo.')}finally{setSaving(false)}}
  return <section className="rounded-2xl border border-red-900/40 bg-red-950/10 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-black">Imagem de fundo da cartela</h3><p className="mt-1 text-xs text-slate-400">Adicione sua arte e ajuste na própria tela de criação, sem abrir outra página.</p></div><Button onClick={()=>setOpen(v=>!v)}>{initial.artwork?.path?'Ajustar imagem de fundo':'Adicionar imagem de fundo'}</Button></div>
  {!open&&<div className="mt-4"><CardTemplatePreview format={format} layoutKey={template.layout_key} eventName={eventName} seriesCode={series.trim().toUpperCase()||'A'} sequenceNumber={startNumber} codePadding={padding} rule={rule} {...previewPropsFromTemplate(template)}/><p className="mt-2 text-center text-xs text-slate-500">Visualização antes de gerar. Os números são apenas uma amostra; a arte e o posicionamento são os mesmos usados na cartela.</p></div>}
  {open&&<div className="mt-4 grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(320px,.9fr)]"><div className="space-y-4"><div><p className="text-sm font-semibold">Escolher imagem</p><label className="mt-2 inline-flex min-h-12 w-full cursor-pointer items-center justify-center rounded-2xl bg-red-600 px-4 py-3 text-base font-black text-white transition hover:bg-red-500 focus-within:outline focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-red-400 sm:w-auto"><input className="sr-only" type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>choose(e.target.files?.[0]??null)}/>Buscar imagem no aparelho</label><p className="mt-2 text-xs text-slate-400">{artFile?`Selecionada: ${artFile.name}`:(initial.artwork?.path?'Uma imagem já está salva. Escolha outra somente se quiser substituir.':'Nenhuma imagem selecionada.')}</p></div><div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-semibold">Encaixe<Select className="mt-1" value={fit} onChange={e=>{setFit(e.target.value as ArtworkFit);setZoom(1);setOffsetX(0);setOffsetY(0)}}><option value="cover">Preencher toda a cartela</option><option value="contain">Mostrar a imagem inteira</option></Select></label><label className="text-sm font-semibold">Qualidade<Select className="mt-1" value={quality} onChange={e=>setQuality(e.target.value as ArtworkQuality)}><option value="light">Arquivo leve</option><option value="standard">Impressão padrão</option><option value="high">Alta qualidade</option></Select></label></div><div className="grid gap-3 sm:grid-cols-3"><QuickRange label={`Tamanho ${zoom.toFixed(2)}×`} min={.5} max={3} step={.05} value={zoom} set={setZoom}/><QuickRange label={`Horizontal ${offsetX}%`} min={-60} max={60} step={1} value={offsetX} set={setOffsetX}/><QuickRange label={`Vertical ${offsetY}%`} min={-60} max={60} step={1} value={offsetY} set={setOffsetY}/></div><div className="flex flex-wrap gap-2"><Button variant="secondary" onClick={()=>{setFit('cover');setZoom(1);setOffsetX(0);setOffsetY(0)}}>Centralizar</Button><Button disabled={saving||(!artFile&&!initial.artwork?.path)} onClick={()=>void save()}>{saving?'Salvando…':'Salvar imagem e ajuste'}</Button><Button variant="secondary" onClick={()=>setOpen(false)}>Cancelar</Button></div>{message&&<p className="rounded-xl bg-slate-950/30 p-3 text-xs font-semibold text-slate-300">{message}</p>}</div><div><p className="mb-2 text-xs font-black uppercase tracking-[.14em] text-red-400">Visualização ao vivo</p><CardTemplatePreview format={format} layoutKey={template.layout_key} eventName={eventName} seriesCode={series.trim().toUpperCase()||'A'} sequenceNumber={startNumber} codePadding={padding} rule={rule} previewOptions={options} localArtworkUrl={artUrl??existingUrl}/><p className="mt-2 text-xs text-slate-500">Ajuste até os jogos encaixarem na arte. Salve quando estiver correto.</p></div></div>}</section>
}
