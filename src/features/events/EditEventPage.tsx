...
const initial:Partial<EventFormValues>={game_mode:event.game_mode,name:event.name,slug:event.slug,description:event.description??'',location_name:event.location_name??'',address:event.address??'',starts_at:toLocalDateTimeInput(event.starts_at),ends_at:toLocalDateTimeInput(event.ends_at),sales_open_at:toLocalDateTimeInput(event.sales_open_at),sales_close_at:toLocalDateTimeInput(event.sales_close_at),default_card_price:Number(event.settings.default_card_price)}
