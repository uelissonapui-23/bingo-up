...
export function NewEventPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-6"><div><p className="text-sm font-semibold text-emerald-700">Novo evento</p><h1 className="mt-1 text-3xl font-black">Criar evento</h1><p className="mt-2 text-sm text-slate-600">Escolha bingo numérico, rifa ou bingo de símbolos. Cada modo mantém seus dados e sorteios isolados.</p></div>{error&&<div className="rounded-2xl bg-red-50 p-4 text-sm font-medium text-red-700">{error}</div>}<Card><EventForm onSubmit={submit} busy={busy} submitLabel="Criar evento"/></Card></div>
}
