...
export function DrawHomePage(){
  ...
  return <div className="space-y-6"><div><p className="text-sm font-semibold text-emerald-700">Operação</p><h1 className="mt-1 text-3xl font-black tracking-tight">Sorteio</h1><p className="mt-2 text-sm text-slate-600">Abra o sorteio do evento correto. Cada evento mantém sua sessão e histórico isolados.</p></div>{error&&<div className="rounded-2xl bg-red-50 p-4 text-sm text-red-700">{error}</div>}{loading?<Card>Carregando eventos…</Card>:items.length===0?<Card>Nenhum evento disponível.</Card>:<div className="grid gap-4 md:grid-cols-2">{items.map(item=><Card key={item.id}><div className="flex items-start justify-between gap-3"><div><h2 className="font-black">{item.name}</h2><p className="mt-1 text-sm text-slate-500">{eventStatusLabel[item.status as keyof typeof eventStatusLabel]??item.status}</p></div><div className="flex flex-wrap gap-2"><Link to={`/eventos/${item.id}/sorteio`}><Button>{item.status==='drawing'||item.status==='paused'?'Continuar sorteio':item.status==='finished'?'Ver sorteio':'Iniciar sorteio'}</Button></Link><Link to={`/eventos/${item.id}/painel-publico/configuracao`}><Button variant="secondary">Aparência da TV</Button></Link></div></div></Card>)}</div>}</div>
}
