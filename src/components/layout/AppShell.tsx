...
export function AppShell() {
  ...
  return <><OfflineBanner/><PwaStatus/><div className="bingoup-app min-h-dvh">
    ...
    <div className="min-w-0 lg:pl-[280px]">
      <header className="bingoup-topbar"><div className="flex items-center gap-3"><NavLink to="/" className="lg:hidden"><img className="h-9 w-auto" src="/brand/bingoup-logo-dark.png" alt="BINGOUP"/></NavLink></div><div className="flex items-center gap-3"><span className="bingoup-phase">Fase 12</span><div className="hidden text-right sm:block"><p className="text-sm font-bold text-white">{currentWorkspace?.name}</p><p className="text-xs text-slate-400">Organizador</p></div><Button variant="secondary" onClick={()=>void signOut()}>Sair</Button></div></header>
      <main className="bingoup-main"><Outlet/></main>
    </div>
    <nav className="bingoup-mobile-nav lg:hidden">{mainNav.filter(([path])=>['/','/eventos','/vendas','/sorteio','/historico'].includes(path)).map(([path,label,icon])=><NavLink key={path} to={path} end={path==='/' } className={({isActive})=>`bingoup-mobile-link ${isActive?'is-active':''}`}><NavIcon name={icon}/><span>{label}</span></NavLink>)}</nav>
  </div></>
}
